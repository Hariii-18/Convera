  ...
  const displayedTranscriptBlocks =
    transcriptView === "normalized"
      ...
      : transcriptView === "translated"
        ...
        : transcriptBlocks;
  // Distinguishes "transcription failed" (offer a retry) from "not
  // transcribed yet" / "still processing" in the transcript tab's empty
  // state below.
  const isTranscriptFailed =
    !isTranscriptLoading && !transcript && processingJob?.status === "failed";
            ...
            emptyAction={
              isTranscriptFailed && processingJob ? (
                <Button
                  ...
                  onClick={() =>
                    guard("manage-meeting", () => handleRetryProcessing())
                  }
                >
